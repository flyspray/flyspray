    ...
    <tr id="rs{$search['id']}" <?php if($search == end($user->searches)): ?>class="last"<?php endif; ?>>
      <td><a href="{$baseurl}?do=index&amp;{Url::query_from_array(unserialize($search['search_string']))}" onmousedown="stopBubble(event);return true;">{$search['name']}</a></td>
      <td class="searches_delete">
        <a href="javascript:deletesearch(event,'{$search['id']}','{$baseurl}')">
