...
switch(Post::val('name')){
	...
	case 'task_type':
		$res=$db->Query('SELECT * FROM {list_tasktype} FROM WHERE (project_id=0 OR project_id=?) AND show_in_list=1 AND status_id=?', array($task['project_id'], $value) );
		if($db->countRows($res)<1){
			header(':', true, 403);
			die(L('invalidvalue'));
		}
	...
	case 'operating_system':
		$res=$db->Query('SELECT * FROM {list_os} FROM WHERE (project_id=0 OR project_id=?) AND show_in_list=1 AND status_id=?', array($task['project_id'], $value) );
		if($db->countRows($res)<1){
			header(':', true, 403);
			die(L('invalidvalue'));
		}
	...
	case 'product_category':
		$res=$db->Query('SELECT * FROM {list_category} FROM WHERE (project_id=0 OR project_id=?) AND show_in_list=1 AND status_id=?', array($task['project_id'], $value) );
		if($db->countRows($res)<1){
			header(':', true, 403);
			die(L('invalidvalue'));
		}
	...
	case 'closedby_version':
		$res=$db->Query('SELECT * FROM {list_version} FROM WHERE (project_id=0 OR project_id=?) AND show_in_list=1 AND status_id=?', array($task['project_id'], $value) );
		if($db->countRows($res)<1){
			header(':', true, 403);
			die(L('invalidvalue'));
		}
