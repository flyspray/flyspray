	...
	case 'checks':
		...
		while ($r = $db->fetchRow($hashtypes)){
			...
			if ($r['l'] == 32 && $r['s'] == 0) {
				...
				$maybe = 'CRYPT_BLOWFISH';
				$display = 'cryptblowfish';
			} elseif ($r['s'] == 1) {
				$maybe = 'other pw hashes';

				if ($r['argon2i'] > 0) {
					$maybe .= ': ' . $r['argon2i'] . ' argon2i';
				}
		...
		$page->assign('pwhashes', $pwhashes);
		$page->assign('warnhash', $warnhash);
		$page->assign('warnhash2', $warnhash2);
		...
		*/
		...
		$wrongtaskcats = $db->query("
			SELECT DISTINCT t.task_id, t.product_category, t.project_id AS tpid, c.project_id AS cpid, t.is_closed,
			t.item_summary, p.project_title, c.category_name AS task_category_name,
			op.project_title as other_project_title
			FROM {tasks} t
			LEFT JOIN {list_category} c ON t.product_category=c.category_id
			LEFT JOIN {projects} p ON t.project_id=p.project_id
			LEFT JOIN {list_category} pc ON c.project_id=pc.project_id AND t.product_category = pc.category_id
			LEFT JOIN {projects} op ON pc.project_id=op.project_id
			WHERE (t.project_id <> c.project_id AND c.project_id >0)
			OR c.project_id IS NULL
