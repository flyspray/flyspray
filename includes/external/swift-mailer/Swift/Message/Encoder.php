...
class Swift_Message_Encoder
{
	...
	function QPEncode($data, $chunk=76, $init_chunk=0, $preserve_address=false, $le="\r\n")
	{
		...
		foreach ($data as $key => $string)
		{
			...
			{
				if ($init_chunk && $init_chunk < (strlen($string)+3)) $ret .= $sep;
				$ret .= $le . $string;
			}
			else
			{
				$string = $this->rawQPEncode($string);
				if ($init_chunk > 3)
				{
				...
				elseif ($init_chunk) $ret .= $sep;
				
				preg_match_all('/.{1,'.($chunk-6).'}([^=]{0,3})?/', $string, $matches);
				
				if (count($matches[0])) $ret .= $le . implode($sep . $le, $matches[0]) . $sep;
			}
			$init_chunk = 0;
		}
	...
	{
		return preg_replace('/[^\x21-\x3C\x3E-\x7E\x09\x20]/e', 'sprintf( "=%02x", ord ( "$0" ) ) ;', $string);
	}
	/**
	...
	function &QPEncodeFile($file, $chunk=76, $le="\r\n")
	{
		...
		$cache =& Swift_CacheFactory::getCache();
		while (false !== $byte = $file->read(8192))
		{
			$next = $this->rawQPEncode($byte);
	...
	function header7BitEncode($data, $chunk=76, $init_chunk=0, $le="\r\n")
	{
		...
		elseif ($init_chunk) $ret .= $le;
		$ret .= chunk_split($data, $chunk-2, $le);
		return trim($ret);
	}
	/**
	...
	{
		if (!$this->is7BitPrintable($str)) $str = $charset . "'" . $language . "'" . urlencode($str);
