...
class Swift_Message_Headers
{
	 ...
	 * @param stirng The header built without attributes
	...
	function buildAttributes($header_line, $header_name)
	{
		...
		foreach ($this->attributes[$header_name] as $attribute => $att_value)
		{
			...
			$encoded = $encoder->rfc2047Encode($att_value, $this->charset, $this->language, $avail_len);
			...
			else //... otherwise list of underneath
			{
				...
				if (count($lines) > 1)
				{
					foreach ($lines as $i => $line)
					{
						...
						$att_copy .= "*" . $i;
						if (strpos($encoded, "'") !== false) $att_copy .= "*"; //And if it's got a ' then it needs another asterisk
