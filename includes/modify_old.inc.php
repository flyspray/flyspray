...
echo "appel modify action=>".Req::val('action')."<br>";
...
switch ($action = Req::val('action'))
{
    ...
    case 'update_list':
        ...
        $listcattype  = Post::val('catlisttype');
        $listposition = Post::val('list_position');
        $listshow     = Post::val('show_in_list');
        $listdelete   = Post::val('delete');
        //var_dump($listcattype);
        //add DC 25/03/2015
        //{{{ (CUSTOMS FIELDS) => $listtype = value, not an array 
        $listtype     = Post::val('list_type');
        echo "update_list:$listtype<br>";
       ...
       /*         $update = $db->Query("UPDATE  $list_table_name
                                      ...
                                      */
                if ($listtype == 'lists') //MENU -> Add List
                {
                	$listproject	= Post::val('list_project');
                	if (!isset($listproject[$id])) {
                		$listproject[$id] = 0;
                	}
                	echo "listcattype=>$id:$listcattype[$id]<br>";
                	$update = $db->Query("UPDATE  $list_table_name
                			SET  $list_column_name = ?, list_position = ?, show_in_list = ?, lists_type = ?
                			WHERE  $list_id = ? AND project_id = ?",
                			array($listnames[$id], intval($listposition[$id]), intval($listshow[$id]) , intval($listcattype[$id]), $id, $listproject[$id]));
                }
                else //STANDARD CASES <> 
                {
                	$update = $db->Query("UPDATE  $list_table_name
