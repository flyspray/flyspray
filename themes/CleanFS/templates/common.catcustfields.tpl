...
//echo Filters::noXSS($lists_id_g.":". $lists_name.":".$catlisttype_g);
$countlines = -1;
$categories = $proj->listCategoriesCustFields($proj->id, $lists_id_g,false, false, false);
$root = $categories[0];
unset($categories[0]);
if (count($categories)) : ?>
<div id="controlBox">
...
<?php endif; ?>
  ...
  <form action="<?php echo Filters::noXSS(CreateURL($do, 'list', $proj->id)); ?>" method="post">
    <table class="list">
      <tr>
        <td>
        ...
        <td title="<?php echo Filters::noXSS(L('categoryparenttip')); ?>">
          ...
          <select id="parent_id" name="parent_id">
            ...
            <?php echo tpl_options($proj->listCategoriesCustFields($proj->id, $lists_id_g,false,false,false), Req::val('parent_id')); ?>
          </select>
        </td>
        <td class="buttons">
